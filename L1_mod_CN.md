...
例如，我用的这个放大镜会把输出图形沿各边缩小到40%。也就是，app里设置输出的100mm x 100mm的图形实际刻出来是40mm x 40mm。面积上来说，缩小到了设置输出的（40x40）/(100x100)=**16%**。设置上来说，因为app里只设最长边长，所以之遥记住40%就好了。比如需要30mm长的图形的话，app里要设置75mm。
...
只看数据的话，Pro的精度是L1的4倍(0.15x0.15mm vs 0.3x0.3mm)。这里是实际雕刻精度对比:

1) **L1 vs Pro雕刻100mm x 100mm尺寸图案**: 测试中L1用（100,50）的设置烧穿了部分测试用纸。Pro的功率没那么大，精度和火力控制更好。在同样的纸上，用（100,60）得到了比较精美的结果。

<img src="images/li_l1_100.jpg" height="400px"><img src="images/li_pro_100.jpg" height="400px">

2) **L1 vs Pro雕刻40mm x 40mm尺寸图案**: 雕刻小图案的对比就比较明显了。Pro完胜。
...
1) L1的输出设置的是100mm x 100mm，经过了放大镜之后，实际雕刻结果是40mm x 40mm。

2) L1雕刻设置从之前的（100,70）降到了最低的（1,1）。不然就会烧穿或者烧糊。
